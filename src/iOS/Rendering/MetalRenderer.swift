...
public class MetalRenderer {
    ...
    public func loadFont(fontName: String, fontSize: Float) -> Bool {
        ...
        guard let fontURL = Bundle.main.url(forResource: fontName, withExtension: "ttf", subdirectory: "fonts") ??
    ...
    private func drawTextRaster(_ text: String,
                                x: Float,
                                y: Float,
                                fontSize: Float,
                                fill: SIMD4<Float>,
        ...
        let key = "raster|\(text)|\(fontSize)|fill:\(fill.x),\(fill.y),\(fill.z),\(fill.w)|outline:\(outline?.widthPx ?? 0)"
        let pad: CGFloat = 4
        var entry = textTextureCache[key]
        if entry == nil {
            // Measure
            let color = CGColor(red: CGFloat(fill.x), green: CGFloat(fill.y), blue: CGFloat(fill.z), alpha: CGFloat(fill.w))
            ...
            guard let ctx = CGContext(data: nil, width: w, height: h, bitsPerComponent: 8, bytesPerRow: 4*w, space: cs, bitmapInfo: CGImageAlphaInfo.premultipliedLast.rawValue) else { return }
            ...
            if let outline = outline, outline.widthPx > 0.5 {
                ...
                ctx.setStrokeColor(red: CGFloat(outline.color.x), green: CGFloat(outline.color.y), blue: CGFloat(outline.color.z), alpha: CGFloat(outline.color.w))
                for run in runArray {
            ...
            // Fill
            ctx.setFillColor(red: CGFloat(fill.x), green: CGFloat(fill.y), blue: CGFloat(fill.z), alpha: CGFloat(fill.w))
            CTLineDraw(line, ctx)
    ...
    public func drawTextCenteredOutlined(_ text: String, x: Float, y: Float, fontSize: Float,
        ...
        log("drawTextCenteredOutlined: center=(\(x),\(y)) size=(\(measured2.width),\(measured2.height)) baselineY=\(baselineY) outlineWidth=\(outlineWidth)", level: .debug)
        drawTextRaster(text,
                       x: x,
                       y: y,
                       fontSize: fontSize,
