...
void BossLevel::Update(float deltaTime)
{
    ...
    if (boss)
    {
        ...
        if (boss->GetCurrentState() == Boss::DEATH && !deathSequenceActive)
        {
            ...
            explosions.push_back(std::make_shared<Explosion>("vfx/blast_small.png", pos1, 1.0f));

            // Explosion 2: BlastSmall at 0.29s (second beat)
            r = radiusDist(engine);
            a = angleDist(engine);
            Vector2 pos2 = { bossCenter.x + r * cosf(a), bossCenter.y + r * sinf(a) };
            explosions.push_back(std::make_shared<Explosion>("vfx/blast_small.png", pos2, 1.0f));

            // Explosion 3: BlastBig at 0.58s (third beat)
            r = radiusDist(engine);
            a = angleDist(engine);
            Vector2 pos3 = { bossCenter.x + r * cosf(a), bossCenter.y + r * sinf(a) };
            explosions.push_back(std::make_shared<Explosion>("vfx/blast_big.png", pos3, 1.0f));
        }
