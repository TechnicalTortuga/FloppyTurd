...
bool ResourceManager::LoadTextureInternal(const std::string& id) {
    ...
    try {
        Texture2D texture = PlatformAPI::GetInstance().LoadTexture(path.c_str());
        if (texture.id != 0) {
...
bool ResourceManager::LoadSoundInternal(const std::string& id) {
    ...
    try {
        Sound sound = PlatformAPI::GetInstance().LoadSound(path.c_str());
        if (sound.id != 0) {
            CachedResource<Sound> cached;
            cached.resource = sound;
            cached.isValid = true;
            cached.path = path;
            cached.memoryUsage = 256 * 1024; // Estimate 256KB for sound
            cached.lastAccessed = 0.0f;
...
bool ResourceManager::LoadMusicInternal(const std::string& id) {
    ...
    try {
        Music music = PlatformAPI::GetInstance().LoadMusic(path.c_str());
        if (music.id != 0) {
...
bool ResourceManager::LoadFontInternal(const std::string& id) {
    ...
    try {
        Font font = PlatformAPI::GetInstance().LoadFont(path.c_str());
...
void ResourceManager::LogRegisteredResources(int maxCount) const {
    int actualCount = std::min(maxCount, static_cast<int>(resourceRegistry.size()));
    TraceLog(LOG_INFO, "Registered resources (%d of %d):", actualCount, static_cast<int>(resourceRegistry.size()));
    
    int count = 0;
    for (const auto& pair : resourceRegistry) {
        if (count >= maxCount) break;
        TraceLog(LOG_INFO, "  %s -> %s", pair.first.c_str(), pair.second.relativePath.c_str());
        count++;
    }
}
